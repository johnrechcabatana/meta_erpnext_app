...
class SalesInvoice(SellingController):
	...
	def on_submit(self):
		...
		if taxes and len(taxes) > 0:
			for d in taxes:
				if d.divide_it_by_365_days:
					d.unit_price = ((d.rate / 100) * self.total) / 365
					d.tax_amount = d.unit_price * d.days
					d.base_tax_amount = d.tax_amount * self.conversion_rate
					d.tax_amount_after_discount_amount = d.tax_amount
					d.base_tax_amount_after_discount_amount = d.base_tax_amount
				else:
					d.unit_price = self.total * (d.rate / 100)
					d.tax_amount = d.unit_price
				sum_total += d.tax_amount
				base_sum_total += d.tax_amount * self.conversion_rate
			self.total_taxes_and_charges = sum_total
			self.base_total_taxes_and_charges = base_sum_total
			self.grand_total = self.total + sum_total
			self.base_grand_total = self.base_total + base_sum_total
			self.rounded_total = self.grand_total
			self.base_rounded_total = self.base_grand_total
			self.in_words = frappe.utils.money_in_words(self.rounded_total, self.currency, self.price_list_currency)
			self.base_in_words = frappe.utils.money_in_words(self.base_rounded_total, self.currency,
															 self.price_list_currency)
			frappe.db.set(self, 'total_taxes_and_charges', sum_total)
			frappe.db.set(self, 'base_total_taxes_and_charges', base_sum_total)
			frappe.db.set(self, 'grand_total', self.total + sum_total)
			frappe.db.set(self, 'base_grand_total', self.base_total + base_sum_total)
			frappe.db.set(self, 'rounded_total', self.total + sum_total)
			frappe.db.set(self, 'base_rounded_total', self.base_total + base_sum_total)
			frappe.db.set(self, 'in_words', self.in_words)
			frappe.db.set(self, 'base_in_words', self.base_in_words)
		# End Code
	...
	def on_update(self):
		...
		if taxes and len(taxes) > 0:
			...
			self.base_rounded_total = self.base_grand_total
			self.in_words = frappe.utils.money_in_words(self.rounded_total, self.currency, self.price_list_currency)
			self.base_in_words = frappe.utils.money_in_words(self.base_rounded_total, self.currency, self.price_list_currency)
			frappe.db.set(self, 'total_taxes_and_charges', sum_total)
			frappe.db.set(self, 'base_total_taxes_and_charges', base_sum_total)
			frappe.db.set(self, 'grand_total', self.total + sum_total)
			frappe.db.set(self, 'base_grand_total', self.base_total + base_sum_total)
...
def make_delivery_note(source_name, target_doc=None):
	...
	def update_item(source_doc, target_doc, source_parent):
		...
		sum_total = 0
		base_sum_total = 0
		taxes = source_doc.get("taxes")
		if taxes and len(taxes) > 0:
			for d in taxes:
				if d.divide_it_by_365_days:
					d.unit_price = ((d.rate / 100) * source_doc.total) / 365
					d.tax_amount = d.unit_price * d.days
					d.base_tax_amount = d.tax_amount * source_doc.conversion_rate
					d.tax_amount_after_discount_amount = d.tax_amount
					d.base_tax_amount_after_discount_amount = d.base_tax_amount
				else:
					d.unit_price = source_doc.total * (d.rate / 100)
					d.tax_amount = d.unit_price
				sum_total += d.tax_amount
				base_sum_total += d.tax_amount * source_doc.conversion_rate
			target_doc.total_taxes_and_charges = sum_total
			target_doc.base_total_taxes_and_charges = base_sum_total
			target_doc.grand_total = source_doc.total + sum_total
			target_doc.base_grand_total = source_doc.base_total + base_sum_total
			target_doc.rounded_total = source_doc.grand_total
			target_doc.base_rounded_total = source_doc.base_grand_total
			target_doc.in_words = frappe.utils.money_in_words(source_doc.rounded_total)
			target_doc.base_in_words = frappe.utils.money_in_words(source_doc.base_rounded_total)
		# End Code
