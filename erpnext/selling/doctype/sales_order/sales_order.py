...
def make_delivery_note(source_name, target_doc=None):
	...
	def update_item(source, target, source_parent):
		...
		if taxes and len(taxes) > 0:
			for d in taxes:
				if d.divide_it_by_365_days:
					d.unit_price = ((d.rate / 100) * source.total) / 365
					d.tax_amount = d.unit_price * d.days
					d.base_tax_amount = d.tax_amount * source.conversion_rate
					d.tax_amount_after_discount_amount = d.tax_amount
					d.base_tax_amount_after_discount_amount = d.base_tax_amount
				else:
					d.unit_price = source.total * (d.rate / 100)
					d.tax_amount = d.unit_price
				sum_total += d.tax_amount
				base_sum_total += d.tax_amount * source.conversion_rate
			target.total_taxes_and_charges = sum_total
			target.base_total_taxes_and_charges = base_sum_total
			target.grand_total = source.total + sum_total
			target.base_grand_total = source.base_total + base_sum_total
			target.rounded_total = source.grand_total
			target.base_rounded_total = source.base_grand_total
			target.in_words = frappe.utils.money_in_words(source.rounded_total)
			target.base_in_words = frappe.utils.money_in_words(source.base_rounded_total)
		# End Code
